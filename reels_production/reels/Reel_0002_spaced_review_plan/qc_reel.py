...
def main() -> int:
    ...
    checks["sources_json_valid"] = len(source_data.get("sources", [])) >= 2
    caption_text = CAPTIONS.read_text(encoding="utf-8")
    checks["captions_present"] = "स्पेस्ड रिव्यू" in caption_text and "14 दिन" in caption_text
    if not VIDEO.exists():
