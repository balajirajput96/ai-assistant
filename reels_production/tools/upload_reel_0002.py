"""Idempotently upload verified Reel 0002 artifacts and record real Drive IDs.

The uploader never overwrites an existing Drive file. It searches the expected
folder first, uploads only missing artifacts, and records returned identifiers.
"""
...
REEL_DIR = ROOT / "reels" / "Reel_0002_spaced_review_plan"
...
def upload_once(path: Path, parent_id: str) -> str:
    ...
    uploaded = gws(
        ...
        "--json", json.dumps({"name": path.name, "parents": [parent_id]}),
    )
    return uploaded["id"]


def main() -> int:
    batch_id = json.loads(DRIVE_STATE.read_text(encoding="utf-8"))["batches"]["Batch_001"]
    reel_folder_id = ensure_folder("Reel_0002_spaced_review_plan", batch_id)
    artifacts = [
        REEL_DIR / "reel_0002.mp4",
        REEL_DIR / "narration_hi.wav",
        REEL_DIR / "script_hi.txt",
        REEL_DIR / "captions_hi.srt",
        REEL_DIR / "sources.json",
        REEL_DIR / "research_notes.md",
        REEL_DIR / "visual_prompts.md",
